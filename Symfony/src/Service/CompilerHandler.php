...
// System.php comes from php-pear
...
class CompilerHandler
{
    ...
    function __construct(PreprocessingHandler $preprocHandl, PostprocessingHandler $postprocHandl, UtilityHandler $utilHandl, LoggerInterface $logger, $objdir)
    {
    ...
    function main($request, $compiler_config)
    {
        ...
        if ($ret_link)
        {
            ...
            $this->compiler_logger->info($this->logger_id." - An error occurred during linking: ".json_encode($originalLinkerOutput));
            ...
            $this->compiler_logger->info($this->logger_id." - Linker reformatted output: ".json_encode($linkerOutput));
            ...
            foreach ($files[$ext] as $file)
            {
                ...
                if (!file_exists("$object_file.o"))
                {
                    ...
                    if (isset($ret_compile) && $ret_compile)
                    {
                        ...
                        $this->compiler_logger->info($this->logger_id." - Clang reported files: ".implode(" | ", array_keys($clangElements)));
                        $gccElements = $this->getGccErrorFileList($avr_output);
                        $this->compiler_logger->info($this->logger_id." - Gcc reported files: ".implode(" | ", array_keys($gccElements)));

                        if (array_diff(array_keys($clangElements), array_keys($gccElements)))
                        {

                            $resp["old_message"] = $output;
                            $this->compiler_logger->info($this->logger_id." - Mismatch between clang and gcc output found.");
                            ...
                            $this->compiler_logger->info($this->logger_id." - Clang reported files after removing asm: ".implode(" | ", array_keys($clangElements)));

                            if (array_diff(array_keys($clangElements), array_keys($gccElements)))
                            {
                                $this->compiler_logger->info($this->logger_id." - Mismatch between clang and gcc output found after removing assembly messages.");
                                ...
                                if (array_diff(array_keys($clangElements), array_keys($gccElements)))
                                {
                                    $this->compiler_logger->info($this->logger_id." - Mismatch between clang and gcc output found after removing assembly/library/core messages.");
                                }
                                else
                                {
                                    $this->compiler_logger->info($this->logger_id." - Clang and gcc issue solved. Both report same files with errors.");
                                }
                                $this->compiler_logger->info($this->logger_id." - Gcc output: ".json_encode($avr_output));
                                $this->compiler_logger->info($this->logger_id." - Clang initial output: ".json_encode($output));
                                $this->compiler_logger->info($this->logger_id." - Clang reformated output: ".json_encode($final_clang_output));
                            ...
                            else
                            {
                                $this->compiler_logger->info($this->logger_id." - Gcc output: ".json_encode($avr_output));
                                $this->compiler_logger->info($this->logger_id." - Clang initial output: ".json_encode($output));
                                $this->compiler_logger->info($this->logger_id." - Clang reformated output: ".json_encode($next_clang_output));
            ...
            )
            {
                ...
                if ($header_found === false)
                {
                    ...
                    )
                    {
                        ...
                        {
                            $this->compiler_logger->info($this->logger_id." - Clang reports library issue.");
                            $libFound = true;
                        }
                        ...
                        {
                            $this->compiler_logger->info($this->logger_id." - Clang reports core issue.");
                            $coreFound = true;
                        }
                        if ((strpos($header, "in asm") !== false || strpos($body, "in asm") !== false) && $asmFound === false && $option == "asm")
                        {
                            $this->compiler_logger->info($this->logger_id." - Clang reports assembly issue.");
                            $asmFound = true;
                        }
                        $header = "";
                        $body = "";
